// <copyright file="Port.cs" company="Hubert de Fleurian">
//     Copyright 2018 - Hubert de Fleurian - Licensed under the Apache License 2.0
//     Original work from BrianPeek (https://github.com/BrianPeek/legoev3)
//     See LICENSE.txt file in the project root for full license information.
// </copyright>

namespace Lego.Ev3.Core
{
    using System.ComponentModel;
    using System.Runtime.CompilerServices;
    using System.Threading;

    /// <summary>
    /// An input or output port on the EV3 brick
    /// </summary>
    public sealed class Port : INotifyPropertyChanged
    {
        private readonly SynchronizationContext _context;

        private string _name;
        private DeviceType _type;
        private byte _mode;
        private float _siValue;
        private int _rawValue;
        private byte _percentValue;

        /// <summary>
        /// Initializes a new instance of the <see cref="Port"/> class.
        /// </summary>
        public Port()
        {
            _context = SynchronizationContext.Current;
        }

        /// <summary>
        /// Event fired when a property has changed.
        /// </summary>
        public event PropertyChangedEventHandler PropertyChanged;

        /// <summary>
        /// Gets or sets the name of port.
        /// </summary>
        public string Name
        {
            get
            {
                return _name;
            }

            set
            {
                _name = value;
                NotifyPropertyChanged();
            }
        }

        /// <summary>
        /// Gets or sets the type of device plugged into port.
        /// </summary>
        public DeviceType Type
        {
            get
            {
                return _type;
            }

            set
            {
                _type = value;
                NotifyPropertyChanged();
            }
        }

        /// <summary>
        /// Gets the device mode. Some devices work in multiple modes.
        /// </summary>
        public byte Mode
        {
            get
            {
                return _mode;
            }

            private set
            {
                _mode = value;
                NotifyPropertyChanged();
            }
        }

        /// <summary>
        /// Gets or sets the current International System of Units value associated with the Port.
        /// </summary>
        public float SIValue
        {
            get
            {
                return _siValue;
            }

            set
            {
                _siValue = value;
                NotifyPropertyChanged();
            }
        }

        /// <summary>
        /// Gets or sets the raw value associated with the Port.
        /// </summary>
        public int RawValue
        {
            get
            {
                return _rawValue;
            }

            set
            {
                _rawValue = value;
                NotifyPropertyChanged();
            }
        }

        /// <summary>
        /// Gets or sets the percentage value associated with the Port.
        /// </summary>
        public byte PercentValue
        {
            get
            {
                return _percentValue;
            }

            set
            {
                _percentValue = value;
                NotifyPropertyChanged();
            }
        }

        internal int Index { get; set; }

        internal InputPort InputPort { get; set; }

        /// <summary>
        /// Set the connected sensor's mode
        /// </summary>
        /// <param name="mode">The requested mode.</param>
        public void SetMode(byte mode)
        {
            Mode = mode;
        }

        /// <summary>
        /// Set the connected sensor's mode
        /// </summary>
        /// <param name="mode">The requested mode.</param>
        public void SetMode(TouchMode mode)
        {
            Mode = (byte)mode;
        }

        /// <summary>
        /// Set the connected sensor's mode
        /// </summary>
        /// <param name="mode">The requested mode.</param>
        public void SetMode(NxtLightMode mode)
        {
            Mode = (byte)mode;
        }

        /// <summary>
        /// Set the connected sensor's mode
        /// </summary>
        /// <param name="mode">The requested mode.</param>
        public void SetMode(NxtSoundMode mode)
        {
            Mode = (byte)mode;
        }

        /// <summary>
        /// Set the connected sensor's mode
        /// </summary>
        /// <param name="mode">The requested mode.</param>
        public void SetMode(NxtUltrasonicMode mode)
        {
            Mode = (byte)mode;
        }

        /// <summary>
        /// Set the connected sensor's mode
        /// </summary>
        /// <param name="mode">The requested mode.</param>
        public void SetMode(NxtTemperatureMode mode)
        {
            Mode = (byte)mode;
        }

        /// <summary>
        /// Set the connected sensor's mode
        /// </summary>
        /// <param name="mode">The requested mode.</param>
        public void SetMode(MotorMode mode)
        {
            Mode = (byte)mode;
        }

        /// <summary>
        /// Set the connected sensor's mode
        /// </summary>
        /// <param name="mode">The requested mode.</param>
        public void SetMode(ColorMode mode)
        {
            Mode = (byte)mode;
        }

        /// <summary>
        /// Set the connected sensor's mode
        /// </summary>
        /// <param name="mode">The requested mode.</param>
        public void SetMode(UltrasonicMode mode)
        {
            Mode = (byte)mode;
        }

        /// <summary>
        /// Set the connected sensor's mode
        /// </summary>
        /// <param name="mode">The requested mode.</param>
        public void SetMode(GyroscopeMode mode)
        {
            Mode = (byte)mode;
        }

        /// <summary>
        /// Set the connected sensor's mode
        /// </summary>
        /// <param name="mode">The requested mode.</param>
        public void SetMode(InfraredMode mode)
        {
            Mode = (byte)mode;
        }

        private void NotifyPropertyChanged([CallerMemberName] string propertyName = null)
        {
            PropertyChangedEventHandler handler = PropertyChanged;
            if (handler != null)
            {
                if (_context == SynchronizationContext.Current)
                {
                    handler(this, new PropertyChangedEventArgs(propertyName));
                }
                else
                {
                    _context.Post(delegate { handler(this, new PropertyChangedEventArgs(propertyName)); }, null);
                }
            }
        }
    }
}