...
ngrok.connect(process.env.PORT || 3000, (err, url) => {
  ...
  // Write to action metadata file
  actionMeta.conversations.MAIN_CONVERSATION.url = url;
  fs.writeFile("./action.json", JSON.stringify(actionMeta), (err) => {
    if (err) {
      return console.error(err);
    }
    console.log("[FS] Rewrote action.json to update URL");
    // Update GACTIONS for local dev.
    exec(`gactions update --action_package action.json --project jasper-e5818`, (err, stdout, stderr) => {
      if (err) {
        // Failed
        return console.error(err);
      }
